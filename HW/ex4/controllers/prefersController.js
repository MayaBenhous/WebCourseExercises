...
exports.prefersController = {
  ...
  async getSelectedVocation(req, res) {
    ...
    try {
    const [countResult] = await connection.execute(`SELECT COUNT(*) as perferCount FROM tbl_40_preferences`);
    ...
    if(perfersCount == '5') {
      ...
      const [rows] = await connection.execute(`SELECT * FROM tbl_40_preferences`);  
      if(rows != null) {
        res.status(201).json({success: `Connection success! - View All Prefers:`, rows});                   
        }
      else {
        res.status(400).json({ error: "Table is null." });
        }
